...
void sha384Init(Sha384Context *context)
{
   ...
   context->uDigest.h[0] = 0xCBBB9D5DC1059ED8;
   context->uDigest.h[1] = 0x629A292A367CD507;
   context->uDigest.h[2] = 0x9159015A3070DD17;
   context->uDigest.h[3] = 0x152FECD8F70E5939;
   context->uDigest.h[4] = 0x67332667FFC00B31;
   context->uDigest.h[5] = 0x8EB44A8768581511;
   context->uDigest.h[6] = 0xDB0C2E0D64F98FA7;
   context->uDigest.h[7] = 0x47B5481DBEFA4FA4;
...
void sha384Final(Sha384Context *context, uint8_t *digest)
{
   ...
   if(digest != NULL)
   {
      osMemcpy(digest, context->uDigest.digest, SHA384_DIGEST_SIZE);
   }
}
...
void sha384FinalRaw(Sha384Context *context, uint8_t *digest)
{
   ...
   for(i = 0; i < 8; i++)
   {
      context->uDigest.h[i] = betoh64(context->uDigest.h[i]);
   }
}
