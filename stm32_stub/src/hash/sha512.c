...
void sha512Init(Sha512Context *context)
{
   ...
   context->uDigest.h[0] = 0x6A09E667F3BCC908;
   context->uDigest.h[1] = 0xBB67AE8584CAA73B;
   context->uDigest.h[2] = 0x3C6EF372FE94F82B;
   context->uDigest.h[3] = 0xA54FF53A5F1D36F1;
   context->uDigest.h[4] = 0x510E527FADE682D1;
   context->uDigest.h[5] = 0x9B05688C2B3E6C1F;
   context->uDigest.h[6] = 0x1F83D9ABFB41BD6B;
   context->uDigest.h[7] = 0x5BE0CD19137E2179;
...
void sha512Update(Sha512Context *context, const void *data, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      osMemcpy(context->uBuffer.buffer + context->size, data, n);
...
void sha512Final(Sha512Context *context, uint8_t *digest)
{
   ...
   context->uBuffer.w[14] = 0;
   context->uBuffer.w[15] = htobe64(totalSize);
   ...
   for(i = 0; i < 8; i++)
   {
      context->uDigest.h[i] = htobe64(context->uDigest.h[i]);
   }

   //Copy the resulting digest
   if(digest != NULL)
   {
      osMemcpy(digest, context->uDigest.digest, SHA512_DIGEST_SIZE);
   }
}
...
void sha512ProcessBlock(Sha512Context *context)
{
   ...
   uint64_t a = context->uDigest.h[0];
   uint64_t b = context->uDigest.h[1];
   uint64_t c = context->uDigest.h[2];
   uint64_t d = context->uDigest.h[3];
   uint64_t e = context->uDigest.h[4];
   uint64_t f = context->uDigest.h[5];
   uint64_t g = context->uDigest.h[6];
   uint64_t h = context->uDigest.h[7];

   //Process message in 16-word blocks
   uint64_t *w = context->uBuffer.w;
   ...
   context->uDigest.h[0] += a;
   context->uDigest.h[1] += b;
   context->uDigest.h[2] += c;
   context->uDigest.h[3] += d;
   context->uDigest.h[4] += e;
   context->uDigest.h[5] += f;
   context->uDigest.h[6] += g;
   context->uDigest.h[7] += h;
}
